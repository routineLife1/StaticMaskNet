...
class IFNet(nn.Module):
    ...
    def forward(self, x, timestep=0.5, scale_list=[8, 4, 2, 1], training=False, ensemble=False):
        if training == False:
            channel = x.shape[1] // 2
            img0 = x[:, :channel]
            img1 = x[:, channel:]
        ...
        for i in range(4):
            if flow is None:
                flow, mask = block[i](torch.cat((img0[:, :3], img1[:, :3], f0, f1, timestep), 1), None,
                                      scale=scale_list[i])
                if ensemble:
                    f_, m_ = block[i](torch.cat((img1[:, :3], img0[:, :3], f1, f0, 1 - timestep), 1), None,
                                      scale=scale_list[i])
                ...
                fd, m0 = block[i](torch.cat((warped_img0[:, :3], warped_img1[:, :3], wf0, wf1, timestep, mask), 1),
